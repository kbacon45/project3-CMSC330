  ...
  let nondet =
    List.fold_left
      (*(fun res (q, c, _) ->
        ...
        | Some _ ->
            let others =
              List.filter (fun {input; states} -> (fst s.states) = (fst states) && s.input = input) m.delta
            in
...
let str_of_int_lst_fsm fsm = 
  ...
  let trans = "trans: [\n\t" ^ 
  (string_of_list (fun s -> ((string_of_list string_of_int "" (fst s.states) ";") ^ "-" ^ 
      (match s.input with
        None -> "ε"
       |Some(x) -> String.make 1 x
      ) ^ "->" ^ (string_of_list string_of_int "" (snd s.states) ";")) 
    ) "" delta "\n\t") ^
    "]" in
...
let str_of_int_fsm fsm = 
  ...
  let trans = "trans: [\n\t" ^ 
    (string_of_list (fun s -> ((string_of_int (fst s.states)) ^ "-" ^ 
      (match s.input with
        None -> "ε"
       |Some(x) -> String.make 1 x
      ) ^ "->" ^ (string_of_int (snd s.states)))
    ) "" delta "\n\t") ^
    "]" in
  sigstr ^ qsstr ^ q0str ^ fsstr ^ trans
...
let wrapper_assert_dfa m printer =
  let nondet =
    List.fold_left
        ...
        | Some _ ->
            let others =
              List.filter (fun s2 -> (fst s2.states) = (fst s1.states) && s2.input = s1.input) m.delta
            in
            res || List.length others > 1 )
      false m.delta
  in
